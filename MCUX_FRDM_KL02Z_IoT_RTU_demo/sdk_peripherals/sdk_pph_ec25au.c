...
const char *ec25_comandos_at[] = {
			...
			"AT+CGREG?",
			"AT+CEREG?",
			"AT+CGDCONT=1,\"IP\",\"internet.comcel.com.co\"",     //configuracion de APN
			"AT+QIACT=1",
			"AT+QIACT?",
			"AT+QMTOPEN=0,\"52.54.240.23\",1883",					// direccion ip del servidor
...
status_t ec25ProcesarRespuestaAT(uint8_t comando){
	...
	case kFSM_ENVIANDO_QMTPUB_d1_d2:
			//Busca palabra EC25 en buffer rx de quectel
			puntero_ok = (uint8_t*) (strstr((char*) (&ec25_buffer_rx[0]),
					(char*) (ec25_repuestas_at[kAT_QMTPUB_d1_d2])));
	...
	case kFSM_ENVIANDO_MQTT_MSJ_d1_d2:
		//Busca palabra EC25 en buffer rx de quectel
		puntero_ok = (uint8_t*) (strstr((char*) (&ec25_buffer_rx[0]),
				(char*) (ec25_repuestas_at[kAT_MQTT_MSJ_d1_d2])));
...
status_t sensorultrasonico(float volumen_tanq1, float volumen_tanq2){


	sprintf(ec25_mensaje_mqtt,"volumen1,%.2f,volumen2,%.2f,\r\n %c",volumen_tanq1,volumen_tanq2, 0x1A);
	//uart0ImprimirMensaje((uint8_t *)(&comando_at[0]),strlen(comando_at));	//Envia comando AT indicado
...
uint8_t ec25Polling(void){
	...
	case kFSM_ENVIANDO_QMTOPEN:
		printf("Enviando AT+QMTOPEN=0,\"52.54.240.23\",1883:");
		ec25BorrarBufferRX();	//limpia buffer para recibir datos de quectel
	...
	case kFSM_ENVIANDO_QMTPUB_d1_d2:
		...
		ec25BorrarBufferRX();	//limpia buffer para recibir datos de quectel
		ec25EnviarComandoAT(kAT_QMTPUB_d1_d2);
	...
	case kFSM_PROCESANDO_RESPUESTA:
		...
		case kStatus_Success:
			...
			case kFSM_ENVIANDO_QMTCONN:
				ec25_fsm.anterior = ec25_fsm.actual;//almacena el estado actual
 			    ec25_fsm.actual = kFSM_ENVIANDO_QMTPUB_d1_d2;//avanza a enviar nuevo comando al modem
				break;

			case kFSM_ENVIANDO_QMTPUB_d1_d2:
				ec25_fsm.anterior = ec25_fsm.actual;//almacena el estado actual
			    ec25_fsm.actual = kFSM_ENVIANDO_MQTT_MSJ_d1_d2;//avanza a enviar nuevo comando al modem
				break;
